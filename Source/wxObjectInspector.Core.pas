...
type
  IzCustomControl = interface
    function GetIsMouseDown: Boolean;
    function GetIsMouseInControl: Boolean;
    function GetIsVclStyleUsed: Boolean;
    function GetUseStyleBorder: Boolean;
    function GetUseStyleColor: Boolean;
    function GetUseStyleFont: Boolean;
    property IsMouseDown: Boolean read GetIsMouseDown;
    property IsMouseInControl: Boolean read GetIsMouseInControl;
    property IsVclStyleUsed: Boolean read GetIsVclStyleUsed;
    property UseStyleBorder: Boolean read GetUseStyleBorder;
    property UseStyleColor: Boolean read GetUseStyleColor;
    property UseStyleFont: Boolean read GetUseStyleFont;
  end;
...
/// <summary>
  ...
  TzCustomControl = class(TCustomControl, IzCustomControl)
  private
  ...
  TzObjectInspectorBase = class(TzCustomControl)
  private
    ...
    function GetItemOrder(PItem: PzPropertyItem): Integer;
    procedure SetComponent(AValue: TObject);
    procedure SetObjectVisibility(const AValue: TMemberVisibility);
    procedure SetSortByCategory(const AValue: Boolean);
  protected
  ...
  public
    ...
    property VisibleItems: TList<PzPropertyItem> read FVisibleItems;
  ...
  TzObjectInspectorList = class(TzObjectInspectorBase)
  private
  ...
  TzObjectInspectorSizingList = class(TzObjectInspectorList)
  private
    ...
    procedure SetSplitterColor(const AValue: TColor);
  ...
  TzObjectInspectorHeaderList = class(TzObjectInspectorSizingList)
  private
    ...
    procedure SetHeaderPropText(const AValue: string);
    procedure SetHeaderValueText(const AValue: string);
  ...
  TzObjectInspectorScrollList = class(TzObjectInspectorHeaderList)
  private
    ...
    function GetIndexFromPoint(APoint: TPoint): Integer;
  ...
  protected
    ...
    procedure Paint; override;
    procedure PaintBkgnd(ACanvas: TCanvas); virtual;
...
begin
  ...
  if FDropDown then begin
    ...
    if StyleServices.IsSystemStyle then Canvas.Font.Style := [fsBold];
...
procedure TzObjectInspectorEdit.ButtonClick(Sender: TObject; Button: TMouseButton;
...
var
  ...
  listIndex: Integer;
begin
 if not Assigned(FPropertyItem) then Exit;

  str := Text;
  // If the Edit is emptied, then we will always do a value update!
  if (not FTextChanged) and (not str.Trim.IsEmpty) then Exit;
  FTextChanged := False;
  if (ReadOnly) or (FPropertyItem^.IsSet) or (not FPropertyItem^.AssociatedProperty.IsWritable) then Exit;
...
begin
  ...
  if ReadOnly or (FListBox.ItemIndex < 0) then Exit;

  // Value has been set via DoSetValueFromList before, so Exit because no need to do it again.
  if FDefaultSelectedIndex = FListBox.ItemIndex then Exit;
...
begin
  if Assigned(FListBox) and (FListBox.HandleAllocated) then ShowWindow(FListBox.Handle, SW_HIDE);
end;
...
begin
  ...
  if not FPropertyItem.AssociatedProperty.IsWritable then Exit;

  if not ValueManager.HasListBox(FPropertyItem) then begin
    if (ValueManager.HasDialog(FPropertyItem)) then FButton.DropDown := False;
  ...
  // Get list items.
  if not (FListBox is TzObjectInspectorCustomListBox) then begin
  ...
  //Set selected.
  if selIndex > - 1 then FListBox.Selected[selIndex] := True;
...
begin
  ...
  if inspector.ReadOnly then begin
    ...
    if (FPropertyItem^.IsSet) or (FPropertyItem^.IsClass) then Self.ReadOnly := True;
  end;
  // Invoke OnGetItemReadOnly Event, when ReadOnly = False, giving the user an opportunity to
  // override.
  if not Self.ReadOnly then begin
    if Assigned(inspector.OnGetItemReadOnly) then Self.ReadOnly := inspector.OnGetItemReadOnly(FOwnerInspector, FPropertyItem);
  end;
...
begin
  if (not Assigned(FListBox)) or (FListBox.Items.Count = 0) then Exit;
  inspector := TzCustomObjectInspector(FOwnerInspector);
...
begin
  ...
  if (not Assigned(FPropertyItem)) then Exit;
  if (inspector.SelectedIndex < 0) or (inspector.SelectedIndex > inspector.VisiblePropCount) then Exit;
  if not FPropertyItem^.AssociatedProperty.IsWritable then Exit;
  // HasButton must be True!
  if not ValueManager.HasButton(PropertyItem) then Exit;
...
begin
  inherited;
  if Assigned(FListBox) then begin
    if IsWindowVisible(FListBox.Handle) then HideList;
  end;
  DoSetValueFromEdit;
...
begin
  CheckOwnerList;
  Result := nil;
  if (AIndex < 0) or (not MayHaveChild) then Exit;
...
begin
  if IsCategory then Exit(True);
  if (not MayHaveChild) then Exit(False);   // Category, Set, and Class.
...
begin
  if (not Assigned(AssociatedProperty)) or (IsCategory) then Exit(False);
...
begin
  ...
  if IsCategory then Exit;
  if Assigned(Instance) then Result := AssociatedProperty.GetValue(Instance);
end;
...
begin
  ...
  if Self.Value.IsObject then begin
    ...
    if not Self.Value.IsEmpty then begin
      ...
      if ((obj is TComponent) and (TComponent(obj).Name <> '')) then Result := TComponent(obj).Name;
    end;
    Exit;
  end;
...
begin
  Result := CompareText(PzPropertyItem(Item1)^.QualifiedName,
    PzPropertyItem(Item2)^.QualifiedName);
end;
...
var
  ...
  ptr: Pointer;
begin
  for I := 0 to FRecPtrList.Count - 1 do
  begin
...
begin
  Result := PT(nil);
  if Count > 0 then Result := get_Item(0);
end;

function TzRecordList<T, PT>.Last: PT;
begin
  Result := PT(nil);
  if Count > 0 then Result := get_Item(Count - 1);
end;
...
function TzCustomControl.GetIsMouseDown: Boolean;
begin
  Result := FIsMouseDown;
end;

function TzCustomControl.GetIsMouseInControl: Boolean;
var
...
function TzCustomControl.GetIsVclStyleUsed: Boolean;
begin
  Result := StyleServices.Enabled and not StyleServices.IsSystemStyle;
end;

function TzCustomControl.GetUseStyleBorder: Boolean;
begin
  Result := IsVclStyleUsed and (seBorder in StyleElements);
end;

function TzCustomControl.GetUseStyleColor: Boolean;
begin
  Result := IsVclStyleUsed and (seClient in StyleElements);
end;

function TzCustomControl.GetUseStyleFont: Boolean;
begin
...
function TzObjectInspectorBase.GetFloatPreference: TzFloatPreference;
begin
  Result := FFloatPreference;
end;

function TzObjectInspectorBase.GetItemOrder(PItem: PzPropertyItem): Integer;
var
  I: Integer;
  s: string;
begin
...
begin
  ...
  while Assigned(parentItem) do begin
    if wasModified(parentItem) then Exit(True);
...
begin
  ...
  for i := 0 to FVisibleItems.Count - 1 do begin
    ...
    if ItemNeedUpdate(PItem) then Exit(True);
  end;
end;
...
procedure TzObjectInspectorBase.SetComponent(AValue: TObject);
begin
...
procedure TzObjectInspectorBase.SetObjectVisibility(const AValue: TMemberVisibility);
begin
...
procedure TzObjectInspectorBase.SetSortByCategory(const AValue: Boolean);
begin
...
var
  ...
  {$REGION 'EnumProps'}
  procedure EnumProps(AInstance: TObject; AParentItem, ACategoryItem: PzPropertyItem;
    AQualifiedName, AQualifiedType: string);
  var
    lvAllow: Boolean;
  ...
  begin
    ...
    for lvRttiProperty in lvRttiPropertyArray do begin
      ...
      if (AInstance <> component) then lvCategoryIndex := - 1;
      ...
      if Assigned(FOnBeforeAddItem) then lvAllow := FOnBeforeAddItem(Self, lvItem);
      ...
      if not lvAllow then begin
        ...
        if lvItem.IsClass then begin
          lvInstance := nil;
          if not lvItem.Value.IsEmpty then lvInstance := lvItem.Value.AsObject;
...
begin
  ...
  for I := 0 to FItems.Count - 1 do begin
    ...
    if item^.IsCategory then begin
      if FExpandedList.Contains(item.QualifiedName) then makeChildrenVisible;
    end
    else begin
      ...
      if visibleCondition then makeChildrenVisible;
    end;

    if item.Visible or FSaveVisibleItems.Contains(item.QualifiedName) then FVisibleItems.Add(item);
  end;
end;
...
begin
  ...
  if FBorderStyle <> bsNone then Params.Style := Params.Style or WS_BORDER;
end;

procedure TzObjectInspectorList.SetBorderStyle(const Value: TBorderStyle);
begin
...
constructor TzObjectInspectorSizingList.Create(AOwner: TComponent);
const
...
destructor TzObjectInspectorSizingList.Destroy;
begin
  inherited;
end;

procedure TzObjectInspectorSizingList.DrawSplitter(ACanvas: TCanvas);
var
  color: TColor;
begin
  ACanvas.Refresh;
  color := FSplitterColor;
  if UseStyleColor then color := StyleServices.GetStyleColor(scSplitter);
  ACanvas.Pen.Color := color;
  ACanvas.MoveTo(FSplitterPos, 0);
  ACanvas.LineTo(FSplitterPos, Height);
end;

function TzObjectInspectorSizingList.GetSplitterRect: TRect;
const
...
procedure TzObjectInspectorSizingList.InvalidateNC;
begin
  // The WM_NCPAINT message is sent to a window when its frame must be painted.
  if HandleAllocated and not FLockUpdate then SendMessage(Handle, WM_NCPAINT, 0, 0);
end;

procedure TzObjectInspectorSizingList.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y:
    Integer);
var
  pt: TPoint;
begin
  ...
  FSplitterDown := False;
  if FFixedSplitter then Exit;

  GetCursorPos(pt);
  pt := ScreenToClient(pt);
  pt.Y := 0;

  if SplitterRect.Contains(pt) then FSplitterDown := True;
end;

procedure TzObjectInspectorSizingList.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y:
    Integer);
begin
  inherited;
  if csDesigning in ComponentState then Exit;
...
procedure TzObjectInspectorSizingList.Paint;
begin
  inherited;
  DrawSplitter(Canvas);
end;

procedure TzObjectInspectorSizingList.SetSplitterColor(const AValue: TColor);
begin
...
procedure TzObjectInspectorSizingList.SetSplitterPos(const AValue: Integer);
const
...
procedure TzObjectInspectorSizingList.SplitterPosChanged(var ANewPos: Integer);
begin
  if Assigned(FOnSplitterPosChanged) then FOnSplitterPosChanged(Self, ANewPos);
end;

procedure TzObjectInspectorSizingList.WMMouseMove(var AMessage: TWMMouseMove);
const
  cSplitterPosMargin = 10;
var
  pt: TPoint;
begin
  inherited;
  if (csDesigning in ComponentState) or (FFixedSplitter) then Exit;
...
function TzObjectInspectorHeaderList.GetHeaderPropRect: TRect;
begin
  Result := Rect(0, 0, FSplitterPos, HeaderRect.Height);
end;

function TzObjectInspectorHeaderList.GetHeaderRect: TRect;
begin
  Result := Rect(0, 0, Width, FItemHeight + (FItemHeight div 2));
end;

function TzObjectInspectorHeaderList.GetHeaderValueRect: TRect;
begin
...
procedure TzObjectInspectorHeaderList.SetHeaderPropText(const AValue: string);
begin
...
procedure TzObjectInspectorHeaderList.SetHeaderValueText(const AValue: string);
begin
...
procedure TzObjectInspectorHeaderList.SetShowHeader(const AValue: Boolean);
begin
...
begin
  ...
  if SplitterRect.Contains(pt) then Exit;
  ...
  if HeaderRect.Contains(pt) then begin
    ...
    if Assigned(FOnHeaderMouseDown) then begin
      if FHeaderPropPressed then begin
        ...
        if FHeaderValuePressed then FOnHeaderMouseDown(Self, hpRight, pt.X, pt.Y);
      end;
    end;
  end;
end;
...
begin
  ...
  FItemHeight := Canvas.TextHeight(cDummyText) + cItemHeightMargin; // 17;
end;
...
function TzObjectInspectorScrollList.GetFirstItemIndex: Integer;
begin
...
function TzObjectInspectorScrollList.GetIndexFromPoint(APoint: TPoint): Integer;
var
  R: TRect;
...
begin
  ...
  for i := 0 to MaxItemCount do begin
    ...
    if R.Contains(APoint) then begin
      ...
      if (Result >= FVisibleItems.Count) then Exit(-1);
      Exit;
    end;
  end;
end;

function TzObjectInspectorScrollList.GetItemTop(AIndex: Integer): Integer;
begin
  Result := AIndex * FItemHeight;
  if FShowHeader then Inc(Result, HeaderRect.Height);
end;

function TzObjectInspectorScrollList.GetLastItemIndex: Integer;
begin
  FScrollInfo.cbSize := SizeOf(FScrollInfo);
  FScrollInfo.fMask := SIF_POS;
  GetScrollInfo(Handle, SB_VERT, FScrollInfo);
  Result := min(VisiblePropCount - 1, FScrollInfo.nPos + GetMaxItemCount);
end;

function TzObjectInspectorScrollList.GetMaxItemCount: Integer;
var
  LHeight: Integer;
begin
  LHeight := Height;
  if FShowHeader then Dec(LHeight, HeaderRect.Height);
  Result := LHeight div FItemHeight;
end;

function TzObjectInspectorScrollList.GetVisiblePropCount: Integer;
begin
...
begin
  Result := AIndex - GetFirstItemIndex;
end;
...
procedure TzObjectInspectorScrollList.PaintBkgnd(ACanvas: TCanvas);
var
  LColor: TColor;
begin
  ACanvas.Refresh;
  LColor := Color;
  if UseStyleColor then LColor := StyleServices.GetStyleColor(scWindow);
...
begin
  ...
  FScrollInfo.nPage := GetMaxItemCount;
...
begin
  ...
  if FShowHeader and (ContainsWindow(GetCaretWnd, Handle)) and IsCaretVisible then
    HideCaret(0);
  ...
  case AMessage.ScrollCode of
    ...
    SB_THUMBTRACK: { VCL Style Support ! }
      begin
        if StyleServices.Available and (not StyleServices.IsSystemStyle) then begin
          ...
          DoScrollWindow(0, FItemHeight * (FPrevScrollPos - FScrollInfo.nPos));
    ...
    SB_ENDSCROLL:
      begin
        ...
        if FShowHeader and (ContainsWindow(GetCaretWnd, Handle)) and not IsCaretVisible then begin
  ...
  if (FScrollInfo.nPos <> YPos) then
    DoScrollWindow(0, FItemHeight * (YPos - FScrollInfo.nPos));
...
begin
  inherited;
  if csDesigning in ComponentState then Component := Self;
  ...
  if not (csDesigning In ComponentState) then FPropInspEdit.Parent := Self;
...
begin
  ...
  if (Index > - 1) and (Index = FSelectedIndex) then begin
    ...
    if iOrd > 0 then
      ...
      if (iOrd = 0) and not (PItem.HasChild) then Exit(True);
  end;
end;
...
begin
  ...
  if Result then UpdateProperties(True);
end;
...
begin
  ...
  if Assigned(FOnCollapseItem) then if not FOnCollapseItem(Self, PItem) then Exit(False);
  ...
  for i := 0 to PItem.ChildCount - 1 do begin
    ...
    if Assigned(PChild) then begin
      ...
      if FSaveVisibleItems.Contains(PChild^.QualifiedName) then FSaveVisibleItems.Remove(PChild^.QualifiedName);
    end;
  end;
...
begin
  Result := PItem.HasChild;
  if not Result then Exit;
  if FCircularLinkedProperties.Contains(PItem.QualifiedName) then Exit(False);
  if Assigned(FOnExpandItem) then if not FOnExpandItem(Self, PItem) then Exit(False);
...
begin
  ...
  if Assigned(FOnGetItemReadOnly) then if FOnGetItemReadOnly(Self, PItem) then Exit;
...
var
  M: TWMVScroll;
begin
  if Assigned(FPropInspEdit.ListBox) and IsWindowVisible(FPropInspEdit.ListBox.Handle) then Exit(False);
...
begin
  Result := False;
  if (Index > - 1) and (Index < FVisibleItems.Count - 1) then begin
    if CanFocus then begin
      SelectItem(Index);
      if GetFocus <> Handle then SetFocus;
...
begin
  ...
  FSaveVisibleItems.Clear;
  UpdateItems;
  for i := 0 to FItems.Count - 1 do FItems.Items[i].Visible := True;
  UpdateVisibleItems;
  UpdateScrollBar;
  UpdateEditControl(False);
  Invalidate;
...
function TzCustomObjectInspector.GetExtraRect(Index: Integer): TRect;
var
  w: Integer;
  PItem: PzPropertyItem;
  R: TRect;
...
function TzCustomObjectInspector.GetItemRect(Index: Integer): TRect;
var
  vIndex, Y: Integer;
begin
  Result := TRect.Empty;
...
function TzCustomObjectInspector.GetPlusMinBtnRect(Index: Integer): TRect;
var
  X, Y: Integer;
...
function TzCustomObjectInspector.GetPropTextRect(Index: Integer): TRect;
begin
...
begin
  ...
  if (FSelectedIndex > - 1) and (not FSelItem.IsEmpty) then
    L := FItems.IndexOfQualifiedName(FSelItem.QualifiedName);
  if L > - 1 then
    Result := FItems.Items[L];
end;

function TzCustomObjectInspector.GetValueRect(Index: Integer): TRect;
var
  vIndex, Y: Integer;
begin
  Result := TRect.Empty;
...
function TzCustomObjectInspector.GetValueTextRect(Index: Integer): TRect;
var
  w: Integer;
begin
  ...
  Inc(Result.Left, w);
  if w > 0 then Inc(Result.Left, FSepTxtDis);
end;
...
begin
  ...
  FExtraRectIndex := - 1;
  P := Point(X, Y);
  if SplitterRect.Contains(P) then Exit;
  if FShowHeader and HeaderRect.Contains(P) then Exit;
  if FTrackChange and NeedUpdate then UpdateProperties(); // In case that some property was changed outside of the Inspector !
  ...
  if Index > - 1 then begin
    ...
    if not FTrackChange then begin
      if ItemNeedUpdate(PItem) then begin
        SaveItem := PItem^;
        UpdateProperties;
        Index := FItems.IndexOfQualifiedName(SaveItem.QualifiedName);
    ...
    if (PItem^.HasChild) and (PlusMinBtnRect[Index].Contains(P)) and (not IsItemCircularLink(PItem)) then begin
      ...
      if PItem.Expanded then begin
        if FExpandedList.Contains(PItem^.QualifiedName) then FExpandedList.Remove(PItem^.QualifiedName);
    ...
    if MustUpdate then begin
      ...
      UpdateVisibleItems;
      UpdateScrollBar;
      if Index = FSelectedIndex then Invalidate; // SelectItem will not Invalidate!
    end;

    FClickTime := GetTickCount;
    SelectItem(Index);
    if PItem.IsCategory then Exit;
  end;

  if Index > - 1 then begin
    if not ExtraRect[Index].IsEmpty then begin
      if ExtraRect[Index].Contains(P) then FExtraRectIndex := Index;
    end;
  end;
end;
...
begin
  inherited;
  if not FShowItemHint then Exit;
  P := Point(X, Y);
  Index := GetIndexFromPoint(P);
...
begin
  ...
  if UseStyleColor then LColor := StyleServices.GetStyleColor(scCategoryButtons);
...
begin
  ...
  if Index = GetFirstItemIndex then begin
    FPropsNeedHint := False;
    FValuesNeedHint := False;
  end;
  PItem := FVisibleItems.Items[Index];
  ...
  R := Rect(0, Y, pOrdPos, Y + FItemHeight);
  if Index = FVisibleItems.Count - 1 then R.Height := Height;

  { Background color => will be used to paint property text . }
  LSaveColor := Canvas.Brush.Color;

  LColor := FGutterColor;
  if UseStyleColor then LColor := StyleServices.GetSystemColor(clBtnHighlight);
  Canvas.Brush.Color := LColor;
  Canvas.FillRect(R);

  pmR := PlusMinBtnRect[Index];
  if PItem^.HasChild and (not FCircularLinkedProperties.Contains(PItem^.QualifiedName)) then begin
  ...
  if not PItem^.IsCategory then begin
    if CanDrawChevron(Index) then begin
      ...
      // cY:=R.Top;
      if HasPlusMinus then Dec(X, ValueManager.MinimumPlusSignWidth + 2);
      Canvas.Pen.Color := clWindowText;
      if UseStyleColor then Canvas.Pen.Color := StyleServices.GetSystemColor(clWindowText);
    ...
    if FShowGridLines then begin
      ...
      DrawHorizontalDottedLine(Canvas, pOrdPos + 1, Y, FSplitterPos);
    end;

    if FSelectedIndex = Index then begin
      R := Rect(pOrdPos + 1, Y + 1, FSplitterPos, Y + FItemHeight);
      LColor := FHighlightColor;
      if UseStyleColor then LColor := StyleServices.GetSystemColor(clHighlight);
      Canvas.Brush.Color := LColor;
      Canvas.FillRect(R);
    end
    else begin
    ...
    if (FSelectedIndex = Index) then begin
      ...
      DrawHorizontalDottedLine(Canvas, HorzDotLeft, Y + FItemHeight, Width);
    end
    else begin
      if FShowGridLines then DrawHorizontalDottedLine(Canvas, FSplitterPos, Y, Width);
    end;
  end;
  ...
  if UseStyleColor then LColor := StyleServices.GetStyleColor(scSplitter);
  ...
  if (Index + 1) < FVisibleItems.Count then begin
  ...
  begin
    ...
    if (valueType = vtBool) or (valueType = vtSetElement) then begin
      ...
      if valueType = vtColor then begin
        ...
        ExtraRect := Inspector.ExtraRect[AIndex];
        if Inspector.SelectedIndex = AIndex then InflateRect(ExtraRect, - 1, - 1);
    ...
    if Inspector.UseStyleColor then ValueColor := StyleServices.GetSystemColor(clBtnText);

    ACanvas.Font.Color := ValueColor;
    ACanvas.Font.Style := ACanvas.Font.Style - [fsBold];

    if Inspector.IsValueNotDefault(QualifiedName, ValueName) then begin
      if not Inspector.UseStyleColor then ACanvas.Font.Color := Inspector.NonDefaultValueColor;
      if Inspector.BoldNonDefaultValue then ACanvas.Font.Style := [fsBold];
    end;
...
var
  ...
  begin
    ...
    if UseStyleBorder then InvalidateNC;
  end;
...
begin
  ...
  if (Index < FVisibleItems.Count) then begin
    if (Index <> FSelectedIndex) then begin
      if Assigned(FOnSelectItem) then begin
        if not FOnSelectItem(Self, FVisibleItems.Items[Index]) then Exit;
      end;

      LSI.cbSize := SizeOf(FScrollInfo);
      LSI.fMask := SIF_POS;
      FSelectedIndex := Index;

      if (FSelectedIndex < GetFirstItemIndex) then begin
        { Index out of page => Need to scroll ! }
        LSI.nPos := FSelectedIndex;
        DoSetScrollInfo;
      end
      else begin
        if (FSelectedIndex > GetLastItemIndex - 1) then begin
          { Index out of page => Need to scroll ! }
...
procedure TzCustomObjectInspector.SetAllowSearch(const Value: Boolean);
begin
...
procedure TzCustomObjectInspector.SetBoldNonDefaultValue(const Value: Boolean);
begin
...
procedure TzCustomObjectInspector.SetGridColor(const Value: TColor);
begin
...
procedure TzCustomObjectInspector.SetGutterColor(const Value: TColor);
begin
...
procedure TzCustomObjectInspector.SetGutterEdgeColor(const Value: TColor);
begin
...
procedure TzCustomObjectInspector.SetGutterWidth(const Value: Integer);
begin
  if Value > ValueManager.MaximumGutterWidth then raise EInvalidGutterWidth.Create;
...
procedure TzCustomObjectInspector.SetHighlightColor(const Value: TColor);
begin
...
procedure TzCustomObjectInspector.SetNameColor(const Value: TColor);
begin
...
procedure TzCustomObjectInspector.SetNonDefaultValueColor(const Value: TColor);
begin
...
procedure TzCustomObjectInspector.SetReadOnlyColor(const Value: TColor);
begin
...
procedure TzCustomObjectInspector.SetReferencesColor(const Value: TColor);
begin
...
procedure TzCustomObjectInspector.SetShowGridLines(const Value: Boolean);
begin
...
procedure TzCustomObjectInspector.SetShowGutter(const Value: Boolean);
begin
...
procedure TzCustomObjectInspector.SetSubPropertiesColor(const Value: TColor);
begin
...
procedure TzCustomObjectInspector.SetValueColor(const Value: TColor);
begin
...
begin
  if (Pos < FGutterWidth + 30) then Pos := FGutterWidth + 30;
  if (Pos > ClientWidth  - 30) then Pos := ClientWidth  - 30;
...
begin
  if not FUnRegisterKeys then Exit;
...
begin
  ...
  if Assigned(PItem) then begin
    if PItem^.IsCategory then begin
      if FPropInspEdit.Visible then FPropInspEdit.Visible := False;
      Exit;
    end;
      ...
      if Assigned(parentForm.ActiveControl) then begin
        if (ContainsWindow(parentForm.ActiveControl.Handle, Handle)) then begin
          ...
          if GetCaretWnd = Handle then begin // searching
            ...
            if DoSelectCaret(FSelectedIndex) then Exit; // start search
          end;
        end;
      end;
    end;
...
begin
  if GetCaretWnd = Handle then DestroyCaret;
end;
...
begin
  ...
  if (FExtraRectIndex > - 1) and not (ExtraRect[FExtraRectIndex].IsEmpty) then begin
    if ExtraRect[FExtraRectIndex].Contains(P) then DoExtraRectClick;
  end;
end;
...
begin
  inherited;
  if FAllowSearch then CreateCaret(Handle, 0, 1, FItemHeight - 2);
end;
...
begin
  ...
  case Message.Msg of
    WM_DESTROY:
      begin
        if not (csDesigning in ComponentState) then UnregisterHotKey(Handle, 0);
      end;
    WM_NCLBUTTONDOWN:
      begin
